# congress_mcp_server_enhanced.py
import os
import json
import httpx
...
# Enhanced Bill Resources
@mcp.resource("bills://latest")
...
@mcp.resource("bills://congress/{congress_num}")
...
@mcp.resource("bills://congress/{congress_num}/{bill_type}")
...
async def get_bill_cosponsors(
    ...
    for cosponsor in cosponsors:
        ...
        original_text = " (Original Cosponsor)" if is_original else ""
        result.append(f"- {name} ({party}-{state}), added {date}{original_text}")
...
async def get_bill_committees(
    ...
    if not committees:
        return f"No committees found for {bill_type.upper()} {bill_number} in the {congress}th Congress."
...
async def get_bill_titles(
    ...
    for title in titles:
        ...
        if "chamberName" in title:
            result.append(f"Chamber: {title.get('chamberName', 'Unknown')}")
        
        if "billTextVersionName" in title:
            result.append(f"Version: {title.get('billTextVersionName', 'Unknown')}")
...
async def get_bill_related_bills(
    ...
    for bill in related_bills:
        ...
        bill_type = bill.get("type", "Unknown type")
        bill_number = bill.get("number", "Unknown number")
        bill_congress = bill.get("congress", "Unknown congress")
        
        result.append(f"## {bill_type.upper()} {bill_number} - {bill_congress}th Congress")
        result.append(bill_title)
        
        if "relationshipDetails" in bill:
            result.append("Relationship:")
            for relationship in bill["relationshipDetails"]:
                rel_type = relationship.get("type", "Unknown relationship")
                identified_by = relationship.get("identifiedBy", "Unknown")
...
async def get_bill_summaries(
    ...
    for summary in summaries:
        ...
        result.append(f"## Summary for: {action_desc} ({action_date})")
        result.append(summary.get("text", "No summary text available"))
        result.append(f"Last Updated: {summary.get('updateDate', 'Unknown')}")
        result.append("\n")
...
if __name__ == "__main__":
    ...
    print("Starting Enhanced Congress.gov MCP Server...", file=sys.stderr)
